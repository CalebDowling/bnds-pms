/* eslint-disable */
// ============================================================
// INVENTORY + DELIVERY (lighter screens)
// ============================================================
...
function Inventory() {
  ...
  return (
    <AppShell active="inventory"
      ...
      actions={<>
        <button className="btn btn-secondary btn-sm"><I.Filter className="ic-sm"/> Filter</button>
        <button className="btn btn-secondary btn-sm">Reorder report</button>
        <button className="btn btn-primary btn-sm"><I.Plus/> Receive shipment</button>
      </>}
    >
      <div className="card">
        <table className="tbl">
          <thead><tr>
            <th>NDC</th><th>Product</th><th>Class</th><th>On hand</th><th>Par</th><th>Status</th><th>Location</th><th>Earliest exp</th><th></th>
          </tr></thead>
          <tbody>
            {items.map((x, i) => (
              <tr key={i}>
                <td className="bnds-mono" style={{fontSize: 12, color: 'var(--ink-3)'}}>{x.ndc}</td>
                <td style={{fontWeight: 500}}>{x.name}</td>
                <td><span className="pill">{x.cls}</span></td>
                <td className="t-num" style={{fontWeight: 500}}>{x.onHand}</td>
                <td className="t-num">{x.par}</td>
                <td>
                  {x.status === "ok"  && <span className="pill pill-leaf"><span className="dot dot-ok"/>OK</span>}
                  {x.status === "low" && <span className="pill pill-warn"><span className="dot dot-warn"/>Low</span>}
                  {x.status === "out" && <span className="pill pill-danger"><span className="dot dot-danger"/>Out</span>}
                </td>
