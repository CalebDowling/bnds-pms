...
export async function getIntakeQueue({
  ...
  // Determine sort order — Prisma accepts a single object or an array of objects
  let orderBy:
    | Prisma.IntakeQueueItemOrderByWithRelationInput
    | Prisma.IntakeQueueItemOrderByWithRelationInput[] = {
    receivedAt: "desc" as const,
  };
  if (sort === "priority") {
    orderBy = [
      { priority: "asc" as const }, // STAT first, then urgent, then normal
      { receivedAt: "desc" as const },
    ];
...
export async function getIntakeStatsBySource(): Promise<IntakeSourceStats> {
  const { requireUser } = await import("@/lib/auth");
  const { prisma } = await import("@/lib/prisma");
  await requireUser();
  ...
  // Initialize accumulator with all required keys set to 0,
  // so the return type satisfies the explicit shape.
  const stats: IntakeSourceStats = {
    prescriber_portal: 0,
    walk_in: 0,
    phone: 0,
    fax: 0,
