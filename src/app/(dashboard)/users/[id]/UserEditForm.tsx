...
export default function UserEditForm({ user, roles }: { user: any; roles: any[] }) {
  ...
  if (!editing) {
    return (
      <div className="flex gap-3">
        <button onClick={() => setEditing(true)} className="px-4 py-2 bg-[#1B4F72] text-white text-sm font-medium rounded-lg hover:bg-[#154360]">Edit User</button>
  ...
  return (
    <div className="bg-white rounded-xl border-2 border-[#1B4F72] p-6">
        ...
        <div className="flex flex-wrap gap-2">
          {roles.map((role: any) => (
            <label key={role.id} className={`flex items-center gap-2 px-3 py-2 rounded-lg border cursor-pointer text-sm ${
              selectedRoles.includes(role.id) ? "border-[#1B4F72] bg-blue-50" : "border-gray-200"
            }`}>
      ...
      <div className="flex justify-end gap-2">
        ...
        <button onClick={handleSave} disabled={loading} className="px-6 py-2 bg-[#1B4F72] text-white text-sm font-medium rounded-lg disabled:opacity-50">
