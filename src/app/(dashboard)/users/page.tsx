...
// ── Mock users (mirrors design-reference/screens/users.jsx UsersA) ──
...
type PermVal = "all" | "edit" | "fill" | "view" | "asst" | "no";
...
const PERM_MAP: Record<PermVal, { l: string; c: string; bg: string }> = {
...
function PermCell({ val }: { val: PermVal }) {
  const m = PERM_MAP[val] || PERM_MAP.no;
  return (
    <span
      style={{
        display: "inline-flex",
...
function StaffStatusPill({ s }: { s: StaffStatus }) {
  ...
  return (
    ...
    >
      <Toolbar
        ...
        searchPlaceholder="Search by name, email, role, location…"
        filters={
          tab === "staff"
            ? [
            ...
            <tbody>
              {STAFF.map((u) => (
                ...
                >
                  ...
                  <td className="t-xs">
                    {u.lic ? (
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        ...
                        {u.lic.dea !== "—" && (
                          <span className="bnds-mono" style={{ fontSize: 11.5, color: "var(--ink-3)" }}>
                            · {u.lic.dea}
                          </span>
                        )}
                  ...
                  <td>
                    <StaffStatusPill s={u.status} />
                  </td>
                  <td
                    className="t-xs"
                    style={{
                      color: u.last === "Now" ? "var(--ok)" : "var(--ink-3)",
                      fontWeight: u.last === "Now" ? 500 : 400,
                    }}
                  >
                    {u.last}
                  </td>
            ...
            <tbody>
              {INVITES.map((i, idx) => (
                <tr key={idx}>
                  ...
                  <td>
                    {i.expiring ? (
                      <StatusPill tone="warn" label="Expires in 24h" />
                    ) : (
                      <StatusPill tone="info" label="Pending" />
                    )}
                  </td>
                  <td>
      ...
      {tab === "roles" && (
        <div className="card" style={{ overflow: "hidden" }}>
          ...
          >
            <span>
              <span className="bnds-mono" style={{ fontWeight: 600, color: "var(--bnds-forest)" }}>
                ●●●
              </span>{" "}
              Full
            </span>
