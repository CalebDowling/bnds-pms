...
import { Printer, Download, RotateCcw, ChevronLeft, RefreshCw } from "lucide-react";
...
export default function CompoundLabelEditorPage() {
  ...
  // Auto-generate preview on page load AND whenever form data changes
  useEffect(() => {
    const timeout = setTimeout(() => {
      generateLabel();
    }, hasAutoGenerated.current ? 600 : 0); // debounce 600ms after first load
    hasAutoGenerated.current = true;
    return () => clearTimeout(timeout);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [formData, noClaimWarning, holdWarning]);
    ...
    <div>
      ...
      <div className="px-6 pb-6">
        ...
        <div className="bg-white rounded-lg border border-[var(--border)] p-3 mb-4 flex items-center gap-3 flex-wrap">
          ...
          {loading && (
            <span className="text-xs text-[var(--text-muted)] animate-pulse">Updating preview...</span>
          )}
          {error && (
