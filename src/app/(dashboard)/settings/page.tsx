...
import { DesignPage, I, StatusPill } from "@/components/design";

// ── Settings (mirrors design-reference/screens/settings.jsx SettingsA) ──
type SectionId =
  | "pharmacy"
  | "hours"
  | "devices"
...
// Mini settings icons (separate from rx-themed I.* set)
const SI = {
  Building: (p: IconProps) => (
    <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.6" {...p}>
      <rect x="4" y="3" width="16" height="18" rx="1" />
      <path d="M9 7h2M9 11h2M9 15h2M13 7h2M13 11h2M13 15h2M9 19v2M15 19v2" />
    </svg>
  ),
  Clock: (p: IconProps) => (
    <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.6" {...p}>
  ...
  Bell: (p: IconProps) => (
    <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.6" {...p}>
      <path d="M6 16V11a6 6 0 0 1 12 0v5l1.5 2h-15L6 16zM10 21a2 2 0 0 0 4 0" />
    </svg>
  ),
  ...
  Palette: (p: IconProps) => (
    <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.6" {...p}>
      <path d="M12 3a9 9 0 1 0 4 17 2 2 0 0 0 0-4h-1a2 2 0 0 1 0-4h2a4 4 0 0 0 4-4 9 9 0 0 0-9-5z" />
  ...
  return (
    ...
    >
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 13.5, fontWeight: 500, color: danger ? "var(--danger)" : "var(--ink)" }}>
          {label}
        </div>
        {hint && (
  ...
  return (
    <div
      style={{
        ...
        height: 20,
        borderRadius: 999,
        background: on ? "var(--bnds-forest)" : "var(--ink-4)",
        position: "relative",
        cursor: "pointer",
      }}
    >
...
function TextField({ value, mono }: { value: string; mono?: boolean }) {
  return (
    <div
      style={{
        padding: "6px 10px",
...
function SectionHeader({ title, desc, tight }: { title: string; desc?: string; tight?: boolean }) {
  return (
    <div style={{ marginBottom: tight ? 8 : 14 }}>
      <h3 className="bnds-serif" style={{ fontSize: tight ? 15 : 18, fontWeight: 500, margin: 0 }}>
        {title}
      </h3>
      {desc && (
...
function SectionDevices() {
  const dev: Array<{ n: string; loc: string; s: string; tone: "ok" | "warn" }> = [
  ...
  return (
    <div>
          ...
          <tbody>
            {dev.map((d, i) => (
              <tr key={i}>
                ...
                <td>
                  <StatusPill tone={d.tone} label={d.s} />
                </td>
                <td>
      ...
      <button className="btn btn-secondary btn-sm" style={{ marginTop: 12 }}>
        <I.Plus className="ic-sm" /> Pair new device
      </button>
    </div>
  );
}
...
function SectionIntegrations() {
  const ints: Array<{ n: string; d: string; on: boolean; status: string; tone: "ok" | "mute" }> = [
          ...
          <div key={i} className="card" style={{ padding: 14, display: "flex", alignItems: "flex-start", gap: 12 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div
                ...
                style={{
                  marginTop: 6,
                  color: it.tone === "ok" ? "var(--ok)" : "var(--ink-4)",
                  fontWeight: 500,
                }}
              >
                {it.status}
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        <div className="card card-pad">
          ...
          >
            <span className="bnds-serif" style={{ fontSize: 28, fontWeight: 500 }}>
              BNDS
            </span>
          </div>
          <button className="btn btn-secondary btn-sm" style={{ marginTop: 12 }}>
          ...
          <SRow
            ...
            control={
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                ...
                />
                <span className="bnds-mono" style={{ fontSize: 12 }}>
                  #1F4D2A · forest
                </span>
              </div>
            }
    ...
    <div>
      ...
      <div className="card card-pad" style={{ padding: "4px 18px" }}>
        ...
        <SRow
          ...
          hint="All systems green"
          control={
            <span className="t-xs" style={{ color: "var(--ok)", fontWeight: 500 }}>
              14 minutes ago · 2.4 GB
            </span>
          }
        />
      ...
      <div style={{ marginTop: 14 }}>
        ...
        <div className="card card-pad" style={{ padding: "4px 18px" }}>
          <SRow
            ...
            hint="CSV · all active patients"
            control={
              <button className="btn btn-secondary btn-sm">
                <I.Download className="ic-sm" /> Download
              </button>
            }
          />
          <SRow
            label="Export Rx history"
            hint="CSV · select date range"
            control={
              <button className="btn btn-secondary btn-sm">
                <I.Download className="ic-sm" /> Download
              </button>
          ...
          />
          <SRow
            label="Full data export (HIPAA)"
            hint="Complete data archive — takes 10–20 min to generate"
            control={<button className="btn btn-secondary btn-sm">Request</button>}
          />
        </div>
      </div>
    </div>
...
const SECTION_RENDERERS: Record<SectionId, React.ComponentType> = {
...
export default function SettingsPage() {
  const [active, setActive] = React.useState<SectionId>("pharmacy");
            ...
            return (
              <div
                ...
                style={{
                  ...
                  cursor: "pointer",
                  fontSize: 13.5,
                  background: isActive ? "var(--bnds-leaf-100)" : "transparent",
                  borderLeft: isActive ? "3px solid var(--bnds-forest)" : "3px solid transparent",
                  fontWeight: isActive ? 600 : 400,
                  color: isActive ? "var(--bnds-forest-deep)" : "var(--ink)",
                }}
              >
                <Icon style={{ flexShrink: 0, color: isActive ? "var(--bnds-forest)" : "var(--ink-3)" }} />
                <span>{c.label}</span>
              </div>
            );
          })}
